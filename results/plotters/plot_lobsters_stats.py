...
def add_labels(x,y,plt,color,offset):
    for i in range(len(x)):
        ...
        plt.text(x[i], y[i]+offset, label, ha='center', color=color, size=10)

def add_text_labels(x,y,plt,color,offset):
    for i in range(len(x)):
        plt.text(x[i], offset, y[i], ha='center', color=color, size=10)
...
plt.text(2.7, 168, '\emph{Disguise/Reveal Ops}',
