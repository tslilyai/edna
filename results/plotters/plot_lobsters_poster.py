...
matplotlib.rc('figure', figsize=(4,1.4))
...
###################### shading
plt.axvspan(-0.5, 0.5, color='white', alpha=0, lw=0)
plt.axvspan(0.5, 4.5, color='purple', alpha=0.08, lw=0)
plt.text(3, 210, '\emph{Disguise/Reveal Ops}',
...
plt.bar((X-barwidth/2)[:3], [
    ...
    yerr=get_yerr([frontpage_durs_baseline, delete_durs_baseline, hobby_durs_baseline]),
    capsize=3,
...
plt.bar((X+barwidth/2), [
    ...
    yerr=get_yerr([frontpage_durs, delete_durs, hobby_durs, restore_durs, unhobby_durs]),
    capsize=3,
...
plt.savefig('lobsters_op_stats.png', dpi=300)
