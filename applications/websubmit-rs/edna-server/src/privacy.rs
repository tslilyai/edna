...
pub(crate) fn anonymize_answers(
    ...
    let is_baseline = bg.is_baseline;
    let did = disguises::universal_anon_disguise::apply(&mut *bg, is_baseline).unwrap();
...
pub(crate) fn edit_as_pseudoprincipal(
    ...
    let pps =
        bg.edna
            .get_pseudoprincipals(data.email.to_string(), Some(data.apikey.to_string()), None);
    warn!(
        bg.log,
    ...
    let pps_str = pps
        .iter()
        .filter(|pp| pp.contains("anon"))
        .map(|pp| format!("'{}'", pp))
        .collect::<Vec<String>>()
        .join(",");
    let answers_res = bg.query_iter(&format!(
        "SELECT email, q, answer FROM answers WHERE lec = {} AND email IN ({});",
        data.lec_id, pps_str
    ));
    assert!(!answers_res.is_empty());
...
pub(crate) fn delete_submit(apikey: ApiKey, backend: &State<Arc<Mutex<MySqlBackend>>>) -> Redirect {
    let mut bg = backend.lock().unwrap();
    let is_baseline = bg.is_baseline;
    let did = disguises::gdpr_disguise::apply(
        &mut *bg,
        apikey.user.clone(),
        apikey.key.clone(),
        true,
