...
impl HighLevelAPI {
    ...
    fn reveal_remove_diffs_of_table<Q: Queryable>(
        ...
        match dsmap.get(table) {
            Some(ds) => {
                for (uid, d) in ds {
                    debug!("Reversing remove record {:?}\n", d);
                    let revealed = d.reveal(&table_info, &uid, llapi, conn)?;
                    if revealed {
                        debug!("Remove Record revealed!\n");
                    } else {
                        debug!("Failed to reveal remove record");
                        return Ok(false);
                    }
                }
                Ok(true)
    ...
    pub fn reveal<Q: Queryable>(
        ...
        if uid != None {
            ...
            if let Some(key) = priv_key {
                // PROXY: login user
                if self.crypto {
                    conn.query_drop(format!("LOGIN {}", base64::encode(&key)))
                        .unwrap();
                }
                decrypt_cap = key;
            }
    ...
    pub fn reveal_using_secretkey<Q: Queryable>(
        ...
        // PROXY: login user
        if self.crypto {
            if decrypt_cap.len() > 0 {
                conn.query_drop(format!("LOGIN {}", base64::encode(&decrypt_cap)))
                    .unwrap();
            }
        }
        ...
        for dwrapper in &dts {
            ...
            if dwrapper.did == did && d.typ == REMOVE_PRINCIPAL {
                debug!("Reversing principal remove record {:?}\n", d);
                ...
                if revealed {
                    debug!("Principal Remove Record revealed!\n");
                } else {
                    failed = true;
                    debug!("Failed to reveal remove record");
                }
        ...
        for dwrapper in &dts {
            ...
            if dwrapper.did == did && d.typ != REMOVE_GUISE && d.typ != REMOVE_PRINCIPAL {
                ...
                let revealed = d.reveal(&table_info, &dwrapper.uid, &mut llapi, conn)?;
                if revealed {
                    debug!("NonRemove Diff Record revealed!\n");
                } else {
                    failed = true;
                    debug!("Failed to reveal non-remove record");
                }
            }
        }
    ...
    pub fn apply_using_secretkey<Q: Queryable>(
        ...
        let (_, speaksfor_records, pks) = llapi.get_recs_and_privkeys(&decrypt_cap);
        drop(llapi);

        // PROXY: login all pseudoprincipals
        if self.crypto {
            let mut db = self.pool.get_conn().unwrap();
            for pk in &pks {
                db.query_drop(format!("LOGIN {}", base64::encode(&pk.1.priv_key)))
                    .unwrap();
            }
        }
                    ...
                    if curtable_info.owner_fk_cols.len() == 1 {
                        ...
                        for i in &pred_items {
                                ...
                                .get_mut(&(table.clone(), ids.clone()))
                            {
                                ...
                                debug!("Found shared obj to remove, hasn't been removed yet");
                                ...
                                if !pps.is_empty() {
                                    ...
                                    debug!("Insert shared obj pps query: {}", q);
                                ...
                                for owner_col in &curtable_info.owner_fk_cols {
                                        ...
                                        || disguise.user.as_ref().unwrap() == &curuid
                                    {
                                        ...
                                        debug!("Inserting mapping from {} to {} for shared obj to remove", curuid, newuid);
                                        match self
                                            .removed_shared_objs
                                            .get_mut(&(table.clone(), ids.clone()))
                                        {
                            ...
                            if should_remove {
                                ...
                                debug!(
                                    "Edna: shared remove by updating to anon items {}",
                                    updatestmt,
                                );
        ...
        for (_group, items) in groups {
            for user_fk_col in user_fk_cols {
                ...
                if uid.is_none() || valid_owners.contains(&old_uid) {
                    ...
                    let (privkey, _pubkey) =
                        llapi.register_pseudoprincipal(did, &old_uid, &new_uid, crypto);
                    debug!("Register anon principal {}", new_uid);
                    ...
                    if crypto {
                        db.query_drop(format!("LOGIN {}", privkey)).unwrap();
                    }
                    ...
                    // PROXY: logout pp
                    if crypto {
                        db.query_drop(format!("LOGOUT {}", privkey)).unwrap();
                    }
                }
            }
        }
...
fn modify_items<Q: Queryable>(
    ...
    db.query_drop(q).unwrap();
    warn!(
        "Update column {} table {} for modify: {}",
        col,
        table,
        start.elapsed().as_micros()
    );
