...
impl LowLevelAPI {
    pub fn new(pool: mysql::Pool, in_memory: bool, reset: bool) -> LowLevelAPI {
        let mut db = pool.get_conn().unwrap();
        LowLevelAPI {
            pool: pool,
            record_ctrler: RecordCtrler::new(&mut db, in_memory, reset),
        }
    }
    ...
    pub fn get_sizes(&self, dbname: &str) -> (usize, usize) {
        ...
        self.record_ctrler.get_sizes(&mut db, dbname)
    }
    ...
    pub fn get_pub_key(&self, uid: &UID) -> Option<PublicKey> {
        self.record_ctrler.get_pub_key(uid)
    }
    ...
    pub fn register_principal(&mut self, uid: &UID, password: String, crypto: bool) -> UserData {
        let mut db = self.pool.get_conn().unwrap();
        let user_share = self
            .record_ctrler
            .register_principal_secret_sharing(uid, &mut db, password, crypto);
        user_share
    }

    // registers the princiapl with edna, giving them a private/public keypair
    pub fn register_principal_without_sharing(&mut self, uid: &UID, crypto: bool) -> SecretKey {
        let mut db = self.pool.get_conn().unwrap();
        let privkey = self
            .record_ctrler
            .register_principal_private_key(uid, false, &mut db, true, crypto);
        privkey
    }
    ...
    pub fn end_disguise(&mut self) {
        let mut db = self.pool.get_conn().unwrap();
        self.record_ctrler.save_and_clear_disguise(&mut db);
    }
    ...
    pub fn register_and_save_pseudoprincipal_record(
        ...
        self.register_pseudoprincipal(did, old_uid, new_uid, crypto);
    ...
    pub fn register_pseudoprincipal(
        ...
        let mut db = self.pool.get_conn().unwrap();
        self.record_ctrler
            .register_anon_principal(&old_uid, &new_uid, did, &mut db, crypto)
    }
