...
int
simple_parse_query(void *qip, const char *text, size_t length, int is_utf8)
{
	//STAT_START_TIMER(SQL_PARSER_INIT);
	//MySQLParseUnit pu = PuGeneric;
	MySQLQueryType pu = QtSelect;
	std::set<std::string> cset;
	std::string s("utf8");
	cset.insert(s);
	MySQLSyntaxChecker *rec = new MySQLSyntaxChecker(50708, "", cset);
	bool utf8 = (is_utf8 == 1 ? true : false);
	//STAT_END_TIMER(SQL_PARSER_INIT);

	//STAT_START_TIMER(SQL_PARSER);
	rec->parse(text, length, is_utf8, pu);
	//STAT_END_TIMER(SQL_PARSER);

	//STAT_START_TIMER(SQL_PARSER_DEL);
	delete rec;
	//STAT_END_TIMER(SQL_PARSER_DEL);
