  ...
  : MySQLRecognitionBase(charsets)
{
  ...
  while (true)
  {
    ...
    if (token->type == ANTLR3_TOKEN_EOF || token->type == COMMENT_RULE)
      break;
  }
}
...
MySQLQueryType MySQLQueryIdentifier::getQueryType(const char *text, size_t length, bool is_utf8)
{
  //log_debug2("Starting query type determination\n");
	std::cout << "Starting query type determination\n";
  ...
  //log_debug2("Query type determination done\n");
	std::cout << "Query type determination done\n";
  return result;
}
...
// qapla
void *
qapla_get_parameters(char *text, size_t length, list_t *param_list)
{
	std::set<std::string> cset;
	std::string s("utf8");
	cset.insert(s);
	MySQLScanner *p_scan = new MySQLScanner(text, length, 
			true, 50708, "", cset);
	p_scan->reset();
	...
	MySQLToken token = p_scan->token();
	while ((token.type != ANTLR3_TOKEN_EOF) && (token.type != COMMENT_RULE)) {
#if DEBUG
		printf("%d. %s, type: %d, len: %d, line: %d, charpos: %d\n", i, 
				token.text.c_str(), token.type, token.text.size(),
				token.line, token.position);
#endif
		if (token.type == 549) {
			p = (param_t *) malloc(sizeof(param_t));
			memset(p, 0, sizeof(param_t));
			list_init(&p->param_listp);
			p->line = token.line;
			p->type = token.type;
			p->position = token.position;
			char *str = (char *) token.text.c_str();
			int str_len = token.text.size();
			memcpy(p->str, str, str_len);
			p->len = str_len;
			list_insert(param_list, &p->param_listp);
			//printf("%d. %s, len: %d, line: %d, charpos: %d\n", i, 
			//		token.text.c_str(), token.text.size(),
			//		token.line, token.position);
		}
		p_scan->next();
		token = p_scan->token();
	...
	list_for_each_entry(p, param_list, param_listp) {
		printf("%d. type: %d, %s, len: %d, line: %d, charpos: %d\n", i, p->type,
				p->str, p->len, p->line, p->position);
		i++;
	}
#endif
	return (void *) p_scan;
...
void
qapla_reset_scanner(void *scan)
{
	MySQLScanner *p_scan = (MySQLScanner *) scan;
	delete p_scan;
}
