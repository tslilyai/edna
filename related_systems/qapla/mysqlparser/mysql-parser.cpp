...
void MySQLRecognizer::parse(const char *text, size_t length, bool is_utf8, MySQLParseUnit parse_unit)
{
  ...
  if (d->_input == NULL)
  {
    ...
    d->_input->setUcaseLA(d->_input, ANTLR3_TRUE); // Make input case-insensitive. String literals must all be upper case in the grammar!
		STAT_END_TIMER(MYSQL_INPUT_PROCESS);
		STAT_START_TIMER(MYSQL_INIT_LEXER);
    d->_lexer = MySQLLexerNew(d->_input);
		STAT_END_TIMER(MYSQL_INIT_LEXER);
    d->_lexer->pLexer->rec->state->userp = &d->_context;

		STAT_START_TIMER(MYSQL_GEN_TOKEN);
    d->_tokens = antlr3CommonTokenStreamSourceNew(ANTLR3_SIZE_HINT, TOKENSOURCE(d->_lexer));
		STAT_END_TIMER(MYSQL_GEN_TOKEN);

		STAT_START_TIMER(MYSQL_INIT_PARSER);
    d->_parser = MySQLParserNew(d->_tokens);
		STAT_END_TIMER(MYSQL_INIT_PARSER);
  ...
  else
  {
    ...
    //       mysql_parser_test consumes endless memory with at least one query when reusing the recognizer.
		STAT_START_TIMER(MYSQL_INIT_RESET);
...
//
    ...
    d->_parser->adaptor = ANTLR3_TREE_ADAPTORNew(d->_tokens->tstream->tokenSource->strFactory);
		STAT_END_TIMER(MYSQL_INIT_RESET);
  }
  ...
  case PuCreateTablespace:
    ...
    break;
  default:
		STAT_START_TIMER(MYSQL_RUN_QUERY);
    d->_ast = d->_parser->query(d->_parser).tree;
		STAT_END_TIMER(MYSQL_RUN_QUERY);
    break;
  }
}
...
//--------------------------------------------------------------------------------------------------
void * MySQLRecognizer::get_ast(void)
{
	pANTLR3_BASE_TREE tree = d->_ast;
	return (void *) tree;
}

void MySQLRecognizer::qapla_setup(const char *text, size_t length, bool is_utf8)
{
  // If the text is not using utf-8 (which it should) then we interpret as 8bit encoding
  // (everything requiring only one byte per char as Latin1, ASCII and similar).
  // TODO: handle the (bad) case that the input encoding changes between parse runs.
...
void MySQLRecognizer::qapla_reset(const char *text, size_t length)
{
	//STAT_START_TIMER(MYSQL_INIT_RESET);
	reset();
	d->_input->reuse(d->_input, (pANTLR3_UINT8)d->_text, (ANTLR3_UINT32)d->_text_length, (pANTLR3_UINT8)"");
...
void MySQLRecognizer::qapla_execute(MySQLParseUnit parse_unit)
{
  switch (parse_unit)
  {
  case PuCreateTable:
...
init_parser(void)
{
	...
	std::string s("utf8");
	cset.insert(s);
	MySQLRecognizer *rec = new MySQLRecognizer(50708, "", cset);
	return (void *) rec;
}

void
setup_parser2(void *p, const char *text, size_t length, int is_utf8)
{
	MySQLRecognizer *rec = (MySQLRecognizer *) p;
	bool utf8 = (is_utf8 == 1 ? true : false);
	rec->qapla_setup(text, length, is_utf8);
}

void
run_parser2(void *p, void *qip)
{
	MySQLParseUnit pu = PuGeneric;
	MySQLRecognizer *rec = (MySQLRecognizer *) p;

	//STAT_START_TIMER(SQL_PARSER);
	rec->qapla_execute(pu);
	//STAT_END_TIMER(SQL_PARSER);

#if DEBUG
	std::string pout = rec->dump_tree();
	std::cout << pout << "\n";
#endif
...
void
reset_parser2(void *p, const char *text, size_t length)
{
	MySQLRecognizer *rec = (MySQLRecognizer *) p;
	rec->qapla_reset(text, length);
}

int
run_parser(void *p, void *qip, const char *text, size_t length, int is_utf8)
{
	MySQLParseUnit pu = PuGeneric;
	MySQLRecognizer *rec = (MySQLRecognizer *) p;
	bool utf8 = (is_utf8 == 1 ? true : false);
	...
	//STAT_START_TIMER(SQL_PARSER);
	rec->parse(text, length, is_utf8, pu);
	//STAT_END_TIMER(SQL_PARSER);
...
void *
get_query_ast(void *parser)
{
	MySQLRecognizer *rec = (MySQLRecognizer *) parser;
	void *ast = rec->get_ast();
	return ast;
}
